...
void DFSClient::ProcessCommand(const std::string &command, const std::string &filename) {
    ...
    if (command == "store") {
        ...
        dfs_log(LL_ERROR) << "Invalid command";
...
void DFSClient::Unmount() {
    ...
    for (FileDescriptor fd: descriptors) {
        if (close(fd) != 0) {
            std::cerr << "Could not close file descriptor" << std::endl;
        }
    }
...
void DFSClient::InotifyEventCallback(uint event_type, const std::string &filename, void *data) {
    ...
    try {
        if (!std::regex_match(filename, std::regex(".*\\.(jpg|gif|png|txt|xlsx|docx|psd|md)$"))) {
...
void Usage() {
    std::cout <<
        ...
        "-a, --address <address>:  The server address to connect to (default: 0.0.0.0:14205)\n"
...
int main(int argc, char** argv) {
    ...
    std::string server_address = "0.0.0.0:14205";
